...
def load_and_extract_single_file(ruta):
    """Función auxiliar para extraer características de UN archivo - paralelizable"""
    try:
...
class SimpleSOM:
    ...
    def train(self, data, epochs=100, n_jobs=None):
        """Entrena el SOM con opción de paralelización
        
        n_jobs: número de procesos paralelos (None = automático, 1 = secuencial)
        """
        if n_jobs is None:
            n_jobs = cpu_count()
        ...
        print(f"[v0] Entrenando SOM con {n_jobs} procesos paralelos")
        ...
        for epoch in range(epochs):
            ...
            if n_jobs > 1:
                updates = Parallel(n_jobs=n_jobs)(
                    delayed(self._process_sample)(
                        data[idx], sigma, learning_rate
                    ) for idx in indices
                )
                ...
                for update_info in updates:
                    if update_info is not None:
                        bmu_idx, influence_map, delta = update_info
    ...
    def _process_sample(self, x, sigma, learning_rate):
        """Procesa una muestra para cálculo paralelo"""
        bmu_idx, _ = self.find_bmu(x)
...
def procesar_carpeta_paralelo(carpeta, train_epochs=200, n_jobs=None, n_extract_jobs=None):
    """Procesa FITS y entrena SOM con paralelización
    
    n_jobs: procesos para entrenamiento SOM (None = automático)
    n_extract_jobs: procesos para extracción de características (None = automático)
    """
    if n_jobs is None:
        n_jobs = cpu_count()
    if n_extract_jobs is None:
        n_extract_jobs = cpu_count()
    ...
    # Fase 1: Cargar datos y extraer características EN PARALELO
    print("=" * 80)
    print(f"FASE 1: Extrayendo características espectrales ({n_extract_jobs} procesos)...")
    print("=" * 80)
    
    start_time = time.time()
    
    results = Parallel(n_jobs=n_extract_jobs)(
        delayed(load_and_extract_single_file)(ruta) for ruta in rutas
    )
    ...
    # Fase 2: Entrenar SOM con paralelización
    print("\n" + "=" * 80)
    print(f"FASE 2: Entrenando SOM ({n_jobs} procesos)...")
    print("=" * 80)
    
    som = SimpleSOM(map_width=13, map_height=13, feature_dim=training_data.shape[1], learning_rate=0.5)
    
    start_time = time.time()
    som.train(training_data_norm, epochs=train_epochs, n_jobs=n_jobs)
...
if __name__ == "__main__":
    print("[v0] SOM Paralelizado - Clasificación de Espectros Astronómicos")
    print("[v0] CPU cores disponibles:", cpu_count())
    
    # Usar todos los cores disponibles
    procesar_carpeta_paralelo("data/BINTABLE", train_epochs=100, n_jobs=-1, n_extract_jobs=-1)
